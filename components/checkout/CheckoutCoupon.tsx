    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      {coupon ? (
        <div className="flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 dark:bg-emerald-950/20">
          ...
          <button onClick={removeCoupon} className="flex h-6 w-6 items-center justify-center rounded-full text-emerald-600 transition-colors hover:bg-emerald-200 dark:hover:bg-emerald-900/30" aria-label="Remove coupon">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="default" className="shrink-0" onClick={() => applyCoupon(code)}>Apply</Button>
        </div>
      )}
      <AnimatePresence>
        {couponError && (
